...
func (a *Analyzer) Process(frame []float64, ts time.Time) Features {
	...
	centroidNorm := 0.0
	if normFactor > 0 {
		centroidNorm = clamp(centroid/normFactor, 0, 1)
	}

	rolloff := a.computeRolloff(totalEnergy)
	rolloffNorm := 0.0
	if normFactor > 0 {
		rolloffNorm = clamp(rolloff/normFactor, 0, 1)
	}
	...
	return Features{
		...
		SpectralBalanceLowMid: spectralBalance(bandNorm[0], bandNorm[1]),
		SpectralBalanceMidHi:  spectralBalance(bandNorm[1], bandNorm[2]),
	}
}
...
func (a *Analyzer) computeBandEnergy(totalEnergy float64) ([3]float64, [3]float64) {
	...
	if totalEnergy > 1e-9 {
		for i := range energies {
			normalized[i] = clamp(energies[i]/totalEnergy, 0, 1)
		}
	}
